...
type IconName = 'dashboard' | 'users' | 'tests' | 'items' | 'reports' | 'payments' | 'matrix' | 'settings';
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`admin-shell${collapsed ? ' is-collapsed' : ''}${mobileOpen ? ' mobile-open' : ''}`}>
    ...
    <aside className="sidebar">
      <Brand light/>
      <nav>
        <section><span className="nav-label">Centro de control</span><NavItem icon="dashboard" label="Resumen ejecutivo" href="/admin" active={pathname === '/admin'} closeMobile={closeMobile}/><NavItem icon="users" label="Usuarios" href="/admin/usuarios" active={pathname.startsWith('/admin/usuarios')} closeMobile={closeMobile}/><NavItem icon="tests" label="Evaluaciones" badge="3" closeMobile={closeMobile}/><NavItem icon="items" label="Pruebas y reactivos" href="/admin/pruebas" active={pathname.startsWith('/admin/pruebas')} closeMobile={closeMobile}/></section>
        <section><span className="nav-label">Análisis y operación</span><NavItem icon="reports" label="Resultados y reportes" closeMobile={closeMobile}/><NavItem icon="payments" label="Pagos y catálogo" href="/admin/pagos" active={pathname.startsWith('/admin/pagos')} closeMobile={closeMobile}/><NavItem icon="matrix" label="Normas y matrices" href="/admin/normas" active={pathname.startsWith('/admin/normas')} closeMobile={closeMobile}/></section>
        <section><span className="nav-label">Sistema</span><NavItem icon="settings" label="Configuración" href="/admin/configuracion" active={pathname.startsWith('/admin/configuracion')} closeMobile={closeMobile}/></section>
      </nav>
